...
export function getNewCurlCode({
  ...
  if (detectedPlatform === "powershell") {
    ...
    return `if (-not $env:SOCHFLOW_API_KEY) {
    Write-Error "SOCHFLOW_API_KEY environment variable not found"
    exit 1
}
...
curl --request POST \`
     ...
     --header "x-api-key: $env:SOCHFLOW_API_KEY" \`
     --data $jsonData`;
  } else {
...
if [ -z "$SOCHFLOW_API_KEY" ]; then
    echo "Error: SOCHFLOW_API_KEY environment variable not found. Please set your API key in the environment variables."
    exit 1
fi
...
curl --request POST \\
     --url '${apiUrl}?stream=false' \\
     --header 'Content-Type: application/json' \\
     --header "x-api-key: $SOCHFLOW_API_KEY" \\
